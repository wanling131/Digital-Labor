...
- **后端**：FastAPI 提供 REST 接口，校验 token 后查写 PostgreSQL。
...
| 后端运行时 | Python 3.8 | 建议 3.8+ |
| 后端框架 | FastAPI | REST API，兼容旧接口 |
| 数据库 | PostgreSQL | 业务主库 |
| 认证 | JWT | 7 天有效，payload 含 userId 或 workerId |
| 考勤 Excel | openpyxl + xlrd | 上传解析后入库 |
| 人脸/活体 | 阿里云实人认证（可选） | 配置 ALIYUN_ACCESS_KEY_* 后启用，否则 mock |
| 测试 | Python 测试框架 | 后端 API 自动化测试；Playwright E2E（web/e2e） |
...
│   └── package.json
│
├── server/                   # 后端（Python + FastAPI，端口 3000）
...
**占位或部分完成**：认证管理人脸/活体（后端已接阿里云，前端个人中心已用 FaceVerify 采集）；e 签宝/存证、H5 扫码激活；详见「需求与实现对比」与 `docs/Excel功能点完成情况检查.md`。

**已完成**：合同模板可视化编辑、权限按钮级/数据范围配置。
...
- 详细接口路径见 **`docs/API.md`**、**`server/digital_labor/controllers/`** 中的路由实现，以及 **「功能点/需求与实现对比.md」** 中的测试与说明。
...
2. **再看** `功能点/需求与实现对比.md` 或 `docs/功能点完成情况检查报告.md`：了解与报价表逐项对应关系、测试结果和后续设计。
3. **动手跑**：根目录 `npm run install:all` 后 `npm run dev`，后端 3000、前端 3002；浏览器访问首页后进管理端（/login）或工人端（/h5），按上面流程走一遍。
4. **看代码**：从 `server/scripts/postgres_schema.sql` 或 `database/sqlite_schema.sql`（表结构）、`server/digital_labor/run.py`（入口）、`server/digital_labor/controllers/`（接口）、`web/app/`（Next 页面与布局）按需往下看。
...
| 2 | 后端连通性 | 先启动 API（`npm run dev:api` 或 `python -m digital_labor.run`），在仓库根目录执行 `npm run test:server`，或 `cd server && python scripts/compat_check.py --new http://localhost:3000 --new-only` 通过 |
| 3 | 管理端登录 | 打开 `/pc/login`，错误密码有错误提示；**admin / 123456** 可登录并进入 `/pc/dashboard` |
...
| 8 | 前端 E2E（可选） | 先启动后端；在仓库根目录 `npm run test:e2e` 或在 `web` 目录 `npm run e2e`（见 `tests/frontend/e2e/README.md`），Playwright 用例通过（需本机已安装 Edge） |

**当前状态**：根目录 `npm run test:server` / 黑盒 `npm run test:api:blackbox`、前端 `npm run test:web` 与 E2E（首页/登录/H5）可按文档运行；人脸/活体已接阿里云（可选配置），个人中心人脸采集已打通。

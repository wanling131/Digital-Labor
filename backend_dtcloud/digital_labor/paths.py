...
class Paths:
    ...
    - 旧系统将静态资源暴露在 `/uploads/**`，且 DB 中存的是相对 `server/` 的路径；
      因此新后端继续复用 `server/uploads`，避免前端路径失效。
    """
    ...
    def uploads_root(self) -> str:
        return os.path.join(self.server_root, "uploads")
...
def get_paths() -> Paths:
    # backend_dtcloud/digital_labor/paths.py -> workspace root
    workspace_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    return Paths(workspace_root=workspace_root)
